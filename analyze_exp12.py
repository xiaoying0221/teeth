...
exp12 = load_or_none(r'd:\文档\毕业设计\teeth\runs\exp12_yolov8s_cbam_neck_250ep\results.csv')

print('=' * 76)
print('exp12 YOLOv8s+CBAM(Neck) 训练结果分析')
print('=' * 76)

if exp12 is None:
    print('exp12 结果文件还不存在，请先运行 train_cbam_neck.py')
    print('=' * 76)
    raise SystemExit(0)

print('总轮次: {}'.format(exp12['total_ep']))
print('最终: P={:.4f}  R={:.4f}  mAP50={:.4f}  mAP50-95={:.4f}'.format(
    exp12['last_p'], exp12['last_r'], exp12['last_m50'], exp12['last_m95']))
print('最佳 mAP50={:.4f} @Epoch {}'.format(exp12['best_m50'], exp12['best_m50_ep']))
print('最佳 mAP50-95={:.4f} @Epoch {}'.format(exp12['best_m95'], exp12['best_m95_ep']))
print('后50轮均值={:.4f}  标准差={:.5f}  ({})'.format(
    exp12['mean50'], exp12['std50'],
    '已收敛' if exp12['std50'] < 0.002 else '基本收敛' if exp12['std50'] < 0.003 else '仍震荡'))

print()
print('=' * 76)
print('公平对比：exp8 / exp5 / exp10 / exp11 / exp12')
...
if exp8 is not None:
    ...
    print('CBAM(P4+P5)  mAP50:    {:+.4f}'.format(exp11['best_m50'] - exp8['best_m50']) if exp11 else 'CBAM(P4+P5) 结果不存在')
    print('CBAM(Neck)   mAP50:    {:+.4f}'.format(exp12['best_m50'] - exp8['best_m50']))
    ...
    print('CBAM(P4+P5)  mAP50-95: {:+.4f}'.format(exp11['best_m95'] - exp8['best_m95']) if exp11 else 'CBAM(P4+P5) 结果不存在')
    print('CBAM(Neck)   mAP50-95: {:+.4f}'.format(exp12['best_m95'] - exp8['best_m95']))
