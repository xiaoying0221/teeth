...
print('=' * 72)
print('exp10 YOLOv8s+CBAM(P4) 训练结果分析')
print('=' * 72)

if exp10 is None:
    print('exp10 结果文件还不存在，请先运行 train_cbam_p4.py')
    print('=' * 72)
    raise SystemExit(0)

print('总轮次: {}'.format(exp10['total_ep']))
print('最终: P={:.4f}  R={:.4f}  mAP50={:.4f}  mAP50-95={:.4f}'.format(
    exp10['last_p'], exp10['last_r'], exp10['last_m50'], exp10['last_m95']))
print('最佳 mAP50={:.4f} @Epoch {}'.format(exp10['best_m50'], exp10['best_m50_ep']))
print('最佳 mAP50-95={:.4f} @Epoch {}'.format(exp10['best_m95'], exp10['best_m95_ep']))
print('后50轮均值={:.4f}  标准差={:.5f}  ({})'.format(
    exp10['mean50'], exp10['std50'],
    '已收敛' if exp10['std50'] < 0.002 else '基本收敛' if exp10['std50'] < 0.003 else '仍震荡'))

print()
print('=' * 72)
print('公平对比：exp8 YOLOv8s vs exp5 YOLOv8s+CBAM(全加) vs exp10 YOLOv8s+CBAM(P4)')
print('（同起点：官方预训练；同轮次：250轮）')
print('=' * 72)
print('{:26s} {:>10s} {:>12s} {:>10s} {:>10s} {:>8s}'.format(
...
for name, s in [
    ...
    if s is None:
        print('{:26s} {}'.format(name, '结果不存在'))
    else:
        print('{:26s} {:>10.4f} {:>12.4f} {:>10.4f} {:>10.4f} {:>8d}'.format(
...
if exp8 is not None:
    ...
    print('CBAM(P4)   mAP50:    {:+.4f}'.format(exp10['best_m50'] - exp8['best_m50']))
    print('CBAM(P4)   mAP50-95: {:+.4f}'.format(exp10['best_m95'] - exp8['best_m95']))

if exp5 is not None:
    print()
    print('相对 exp5（P3+P4+P5 全加）的变化:')
    print('CBAM(P4)   mAP50:    {:+.4f}'.format(exp10['best_m50'] - exp5['best_m50']))
    print('CBAM(P4)   mAP50-95: {:+.4f}'.format(exp10['best_m95'] - exp5['best_m95']))
