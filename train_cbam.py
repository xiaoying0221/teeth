...
# -*- coding: utf-8 -*-
"""
口腔影像异常区域检测 - YOLOv8s + CBAM 注意力机制
exp5: 在 Backbone P3/P4/P5 输出后插入 CBAM，从头训练 150 轮
"""
...
MODEL_YAML = Path(r"d:\文档\毕业设计\teeth\yolov8s-cbam.yaml")
...
DEVICE  = "0"      # GPU
WORKERS = 4
NAME    = "exp5_yolov8s_cbam_250ep"


def main():
    print("=" * 60)
    print("口腔影像异常区域检测 - YOLOv8s + CBAM")
    print(f"模型结构: {MODEL_YAML}")
    print(f"Epochs: {EPOCHS}  |  imgsz: {IMGSZ}  |  batch: {BATCH}")
    print(f"设备: GPU {DEVICE}")
    print("=" * 60)

    # 用自定义 YAML 从头初始化（加载结构，预训练权重会自动匹配可用层）
