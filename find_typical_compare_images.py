# -*- coding: utf-8 -*-
"""Find comparison images where exp10 detects more than exp8.
...
- exp10 detects more objects than exp8
- exp8 misses objects that exp10 detects
- exp10 has a prediction count closer to ground truth

Usage example:
    python find_typical_compare_images.py

Outputs:
- comparison_candidates.csv
- comparison_candidates.txt
"""
...
OUT_CSV = BASE / "comparison_candidates.csv"
OUT_TXT = BASE / "comparison_candidates.txt"

# If you also want to require exp10 to be closer to GT than exp8, keep True.
REQUIRE_EXP10_BETTER_THAN_EXP8 = True
...
def load_candidates() -> list[Row]:
    ...
    for stem in all_stems:
        ...
        # score: prefer exp10 > exp8, and exp10 closer to GT count
        score = float(delta)
        if gt_count is not None:
            score += max(0.0, abs(delta_exp8_gt) - abs(delta_exp10_gt))
            score += 0.1 * (gt_count - exp8_count)

        if delta <= 0:
            continue
        if REQUIRE_EXP10_BETTER_THAN_EXP8 and gt_count is not None:
            if abs(delta_exp10_gt) > abs(delta_exp8_gt):
                continue
    ...
    rows.sort(key=lambda r: (r.score, r.delta_exp10_minus_exp8, -(r.gt_count or 0)), reverse=True)
    return rows
    ...
    with OUT_TXT.open("w", encoding="utf-8") as f:
        for i, r in enumerate(rows, start=1):
            f.write(
                ...
                f"| delta={r.delta_exp10_minus_exp8} | score={r.score:.3f}\n"
            )
